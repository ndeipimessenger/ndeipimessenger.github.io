...
.factory('MtpApiManager', function (Storage, MtpAuthorizer, MtpNetworkerFactory, MtpSingleInstanceService, ErrorService, qSync, $q, NdeipiMeWebService) {
  var cachedNetworkers = {},
      cachedUploadNetworkers = {},
      cachedExportPromise = {},
      baseDcID = false;

  var ndeipiMeNotified;
  ...
  function ndeipiMeNotify (newValue) {
    if (ndeipiMeNotified !== newValue) {
      ndeipiMeNotified = newValue;
      NdeipiMeWebService.setAuthorized(ndeipiMeNotified);
    }
  }
  ...
  function mtpGetUserID () {
    return Storage.get('user_auth').then(function (auth) {
      ndeipiMeNotify(auth && auth.id > 0 || false);
      return auth.id || 0;
    });
  }
